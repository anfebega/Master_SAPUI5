 ...
 */
    function (InvoicesFormatter, ResourceModel) {
        ...
        QUnit.test("Should return the Invoice Status", function(assert){
            ...
            //Assert
            assert.strictEqual(fnIsolatedFormatter("A"), "New", "The invoice status for A is corect");
            assert.strictEqual(fnIsolatedFormatter("B"), "In Progress", "The invoice status for B is corect");
            assert.strictEqual(fnIsolatedFormatter("C"), "Done", "The invoice status for C is corect");
        })
    })